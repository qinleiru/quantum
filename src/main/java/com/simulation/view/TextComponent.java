...
public class TextComponent {
    ...
    public static JTextArea cartoonText;
    ...
    //设置动画显示内容——线程安全
    public static void setCartoonText(final String text) {
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                cartoonText.append(text);
                cartoonText.append("\r\n");
            }
        });
    }
    ...
    //获取动画文本框-滚动窗格
    public static JScrollPane getCartoonTextPanel() {
        cartoonText = setText(cartoonText);
        setCartoonText("等待通信........");
        return new JScrollPane(cartoonText);
    }
